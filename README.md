...
<head>
  ...
  <style>
    body { font-family: 'Segoe UI', sans-serif; background: linear-gradient(135deg, #63ad72, #4a7255); margin:0; }
    .box { background: #ffffffcc; padding: 20px; margin: 20px; border-radius: 20px; }
    input, select {
      width: 100%; padding: 10px; margin: 6px 0; border-radius: 10px; border: 1px solid #ddd;
    }
    button {
      width: 100%; padding: 8px; margin: 4px 0; border-radius: 10px; border: none; background: #9ee8ff; font-size: 13px;
    }
    .hidden { display:none; }
    li { display:flex; justify-content:space-between; padding:8px; border-bottom:1px solid #eee; }
    .income { color:rgb(22, 177, 22); }
    .expense { color:red; }
  </style>
</head>
<body>
...
function login(){
  ...
  if(!data[currentUser]) data[currentUser] = { balance: 0, transactions: [] };
...
function showHistory() {
  ...
  trxList.forEach(trx => {
    ...
    li.innerHTML = `<span>${trx.date} - ${trx.desc}</span><span class='${trx.type}'><b>${sign}${formatRupiah(trx.amount)}</b></span>`;
    list.appendChild(li);
  });
...
if(currentUser && data[currentUser]) showMain();
</script>
